...
namespace LoadBalance
{
    ...
    public:
        LoadBalancer() { 
            assert(LoadFromConf()); 
            LOG(NORMAL) << "Successfully loaded LoadBalancer configuration file" << endl;
        }
        ~LoadBalancer() {}
    ...
    private:
        bool LoadFromConf() 
        {
            ...
            vector<string> handleServerIpPort = config._handleServerIpPort;

            string lineMessage;
            for(int i = 0; i < handleServerIpPort.size(); ++i)
            {
                lineMessage = handleServerIpPort[i];
